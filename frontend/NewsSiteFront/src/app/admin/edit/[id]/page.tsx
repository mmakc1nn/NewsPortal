...
export default function EditNewsPage() {
  ...
  const fetchNews = async () => {
    try {
      ...
      setImagePreview(data.imageBase64 || null);
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateNews(id, { title, content, author, imageBase64 });
      router.push('/admin');
    } catch (err: any) {
  ...
  if (!news) {
    return (
      <div className="container mx-auto px-4 py-12 text-center">
        <p className="text-xl text-red-600 mb-4">Новость не найдена</p>
        <button
  ...
  return (
    <div className="container mx-auto px-4 py-12">
      <div className="max-w-3xl mx-auto">
        <h1 className="text-4xl font-bold text-gray-900 mb-8">Редактировать новость</h1>
